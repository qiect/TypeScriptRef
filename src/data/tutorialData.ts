    ...
    sections: [
      {
        ...
        title: "安装 TypeScript",
        content:
          "TypeScript 是 JavaScript 的超集，它在 JavaScript 的基础上添加了静态类型系统。你可以通过 npm 全局安装 TypeScript 编译器，也可以在项目中作为开发依赖安装。安装完成后，使用 tsc 命令即可编译 TypeScript 文件。建议在项目中使用本地安装的方式，这样可以确保团队成员使用相同的 TypeScript 版本。",
        codeExample: {
...
// tsc --init`,
      ...
      },
      {
        ...
        content:
          "tsconfig.json 是 TypeScript 项目的配置文件，它定义了编译器选项和项目包含的文件。通过 tsc --init 可以生成默认的配置文件。常用的配置选项包括 target（编译目标版本）、module（模块系统）、strict（严格模式）、outDir（输出目录）等。开启 strict 模式可以启用所有严格类型检查选项，推荐在新项目中使用。",
        codeExample: {
          code: `{
  "compilerOptions": {
    "target": "ES2020",
  ...
  "exclude": ["node_modules", "dist"]
      ...
      },
      {
        ...
        content:
          "让我们编写第一个 TypeScript 程序。TypeScript 文件以 .ts 为扩展名。与 JavaScript 不同，TypeScript 允许你为变量、函数参数和返回值添加类型注解。编译器会在编译时检查类型是否正确，帮助你提前发现潜在的 bug。编译后的 .js 文件可以在任何 JavaScript 环境中运行。",
        codeExample: {
...
console.log(\`\${newUser.name}, 年龄 \${newUser.age} 岁\`);`,
      ...
      },
      {
        ...
        title: "编译与运行",
        content:
          "TypeScript 代码需要编译成 JavaScript 才能在浏览器或 Node.js 中运行。你可以使用 tsc 命令手动编译，也可以使用 ts-node 直接运行 TypeScript 文件进行开发调试。在开发过程中，使用 watch 模式可以让编译器在文件变化时自动重新编译。对于前端项目，Vite、Webpack 等构建工具也提供了 TypeScript 支持。",
        codeExample: {
          code: `// 手动编译
// tsc index.ts
...
// }`,
  ...
  },
  {
    ...
    title: "基础类型",
    description:
      "掌握 TypeScript 中的基础类型系统，包括原始类型、数组、元组、枚举以及特殊类型。",
    order: 2,
    icon: "Hash",
    sections: [
      {
        ...
        content:
          "TypeScript 的原始类型与 JavaScript 一致，包括 number（数字）、string（字符串）和 boolean（布尔值）。TypeScript 还支持字面量类型，可以将变量的值限定为特定的字面量。类型注解使用冒号语法，如 let age: number = 25。如果变量在声明时已经赋值，TypeScript 可以自动推断类型，此时类型注解可以省略。",
        codeExample: {
...
let move: Direction = "up";`,
      ...
      },
      {
        ...
        title: "数组、元组与枚举",
        content:
          "数组类型有两种写法：使用方括号语法 number[] 或泛型语法 Array<number>。元组（Tuple）是固定长度和类型的数组，每个位置可以有不同类型。枚举（Enum）用于定义一组命名常量，默认从 0 开始编号，也可以手动指定值。反向映射是数字枚举的特性，字符串枚举不支持反向映射。",
        codeExample: {
...
let dir: Direction = Direction.Up;`,
      ...
      },
      {
        ...
        content:
          "void 表示没有返回值，通常用于函数返回类型。null 和 undefined 是所有类型的子类型（strictNullChecks 关闭时）。never 表示永远不会存在的类型，比如总是抛出异常的函数。any 会跳过所有类型检查，应尽量避免使用。unknown 是类型安全的 any，使用前必须进行类型收窄。推荐使用 unknown 替代 any。",
        codeExample: {
...
undefinedValue = 42;`,
      ...
      },
      {
        ...
        title: "类型推断与类型断言",
        content:
          "TypeScript 拥有强大的类型推断能力，在许多情况下你不需要显式标注类型。当变量声明时赋值、函数返回值等场景，编译器会自动推断类型。类型断言用于告诉编译器你比它更了解某个值的类型，使用 as 语法或尖括号语法。但类型断言不会进行运行时检查，滥用可能导致运行时错误。",
        codeExample: {
          code: `let count = 10;
let message = "hello";
let items = [1, 2, 3];
    ...
    sections: [
      {
        ...
        title: "接口定义与使用",
        content:
          "接口是 TypeScript 中定义对象形状的核心方式。它描述了对象应该有哪些属性以及属性的类型。接口只存在于编译时，编译后会被完全移除，不会产生任何运行时代码。接口可以作为参数类型、返回值类型，也可以用来约束变量的结构。当对象字面量直接赋值给接口类型的变量时，会进行严格的多余属性检查。",
        codeExample: {
...
createServer({ host: "localhost", port: 3000, protocol: "http" });`,
      ...
      },
      {
        ...
        title: "可选属性与只读属性",
        content:
          "可选属性使用问号标记，表示该属性可以不存在。这在描述可能缺失的字段时非常有用，比如 API 返回的数据中某些字段可能为空。只读属性使用 readonly 修饰，表示属性只能在对象创建时赋值，之后不能修改。TypeScript 还提供了 Readonly 工具类型，可以将所有属性变为只读。ReadonlyArray 可以创建不可变数组。",
        codeExample: {
...
// readonlyArr.push(4); // 错误: 不允许修改`,
      ...
      },
      {
        ...
        title: "接口继承与合并",
        content:
          "接口支持继承，使用 extends 关键字可以从一个或多个接口派生出新接口。子接口会继承父接口的所有属性，还可以添加新的属性或覆盖同名属性（类型需要兼容）。接口还支持声明合并——同名接口的成员会自动合并。这在扩展第三方库的类型定义时非常有用。",
        codeExample: {
...
const win: Window = {
  ...
  customProp: "扩展属性",
      ...
      },
      {
        ...
        title: "类型别名 type",
        content:
          "类型别名使用 type 关键字定义，可以为任何类型创建一个新名称。与接口不同，类型别名可以表示原始类型、联合类型、交叉类型、元组等任何类型。类型别名不支持继承和声明合并，但可以使用交叉类型来实现类似效果。在实际开发中，接口和类型别名经常配合使用，接口用于定义对象结构，类型别名用于组合和转换类型。",
        codeExample: {
...
const userState: RequestState<{ name: string; age: number }> = {
  ...
  },
  {
    ...
    title: "函数",
    description:
      "深入理解 TypeScript 中函数的类型定义，包括参数类型、返回值、重载和箭头函数。",
    order: 4,
    icon: "Braces",
    sections: [
      {
        ...
        title: "参数类型与返回值类型",
        content:
          "在 TypeScript 中，函数的每个参数都需要指定类型，返回值类型也可以显式声明。虽然 TypeScript 能够根据 return 语句推断返回值类型，但显式声明返回值类型可以让代码意图更清晰，也能防止意外返回错误类型。对于回调函数，参数类型同样需要声明。",
        codeExample: {
...
console.log(executeOperation(multiply, 6, 7));`,
      ...
      },
      {
        ...
        content:
          "可选参数使用问号标记，必须放在必需参数之后。默认参数在参数名后赋予默认值，调用时可以省略。剩余参数使用省略号语法，将多个参数收集为一个数组。注意可选参数和默认参数虽然都可以省略，但语义不同：可选参数的值可能是 undefined，而默认参数在省略时会有确定的默认值。",
        codeExample: {
...
logWithTimestamp("用户登录", { userId: "u001" });`,
      ...
      },
      {
        ...
        title: "函数重载",
        content:
          "函数重载允许同一个函数根据不同的参数类型或数量返回不同类型的值。你需要先声明所有重载签名，然后在最后提供一个统一的实现签名。TypeScript 在调用时会根据传入的参数匹配最合适的重载签名。函数重载在处理多种输入格式时非常有用，比如一个函数既接受字符串也接受对象作为参数。",
        codeExample: {
...
function request<T>(url: string, options?: { responseType: T }): Promise<ApiResponse<T | unknown>> {
  return fetch(url).then((res) => res.json());
      ...
      },
      {
        ...
        title: "箭头函数与函数类型表达式",
        content:
          "箭头函数在 TypeScript 中同样可以使用类型注解。函数类型表达式使用箭头语法定义函数的类型签名。在回调函数和事件处理中，箭头函数类型非常常见。当将箭头函数赋值给已有类型的变量时，TypeScript 可以自动推断参数类型，无需重复声明。",
        codeExample: {
...
const debounce = <T extends (...args: unknown[]) => void>(
  ...
  },
  {
    ...
    title: "类",
    description:
      "学习 TypeScript 中类的定义、访问修饰符、抽象类和泛型类的使用。",
    order: 5,
    icon: "Building",
    sections: [
      {
        ...
        title: "类定义与构造函数",
        content:
          "TypeScript 在 ES6 类的基础上增加了类型注解和访问修饰符。类的属性需要先声明再使用，可以在声明时初始化，也可以在构造函数中赋值。构造函数使用 constructor 关键字定义，参数可以添加类型注解。TypeScript 还支持参数属性，在构造函数参数前添加修饰符可以自动声明并初始化属性。",
        codeExample: {
          code: `class Person {
  name: string;
  age: number;
...
console.log(counter.getCount());`,
      ...
      },
      {
        ...
        title: "访问修饰符与存取器",
        content:
          "TypeScript 提供了三种访问修饰符：public（公开，默认值）、private（私有，仅类内部可访问）和 protected（受保护，类内部和子类可访问）。存取器（getter/setter）可以控制属性的读写行为，在 getter 中计算值，在 setter 中验证值。使用 private 修饰符配合存取器是封装数据的常用模式。",
        codeExample: {
...
console.log(manager.getAnnualSalary());`,
      ...
      },
      {
        ...
        title: "抽象类与实现接口",
        content:
          "抽象类使用 abstract 关键字定义，不能直接实例化。抽象方法只有声明没有实现，子类必须实现所有抽象方法。抽象类适合定义一组相关类的公共行为。类可以使用 implements 关键字实现接口，确保类满足接口定义的契约。一个类可以实现多个接口，同时也可以继承另一个类。",
        codeExample: {
...
console.log(circle.serialize());`,
      ...
      },
      {
        ...
        title: "泛型类",
        content:
          "泛型类使用类型参数来创建可复用的类定义。类型参数在类名后的尖括号中声明，可以在类的属性、方法和构造函数中使用。泛型类在实现数据结构（如栈、队列、链表）时特别有用。可以给类型参数设置默认类型和约束条件，使泛型类更加灵活和安全。",
        codeExample: {
          code: `class Stack<T> {
  private items: T[] = [];
...
console.log(store.get("u1"));`,
  ...
  },
  {
    ...
    title: "泛型",
    description:
      "掌握泛型的核心概念，学习泛型函数、泛型接口、泛型约束以及内置工具类型。",
    order: 6,
    icon: "GitBranch",
    sections: [
      {
        ...
        title: "泛型函数与泛型接口",
        content:
          "泛型允许你编写灵活且可复用的代码，同时保持类型安全。泛型函数在函数名后使用尖括号声明类型参数，调用时可以显式指定类型参数，也可以让 TypeScript 自动推断。泛型接口在接口名后声明类型参数，使得接口可以适配不同的数据类型。泛型在处理集合、API 响应、事件系统等场景中非常常见。",
        codeExample: {
...
interface Repository<T> {
  ...
  delete(id: string): Promise<boolean>;
      ...
      },
      {
        ...
        title: "泛型约束与 keyof",
        content:
          "泛型约束使用 extends 关键字限制类型参数的范围，确保类型参数满足特定条件。常见的约束包括要求类型参数具有某些属性、是某个类型的子类型等。keyof 操作符用于获取类型的所有属性名的联合类型，常与泛型约束配合使用，实现类型安全的属性访问。这在编写通用工具函数时非常有用。",
        codeExample: {
  ...
  emit<K extends keyof T>(event: K, data: T[K]): void {
    this.listeners[event]?.forEach((cb) => cb(data));
      ...
      },
      {
        ...
        title: "内置工具类型",
        content:
          "TypeScript 内置了许多实用的工具类型，它们基于泛型实现，可以快速转换和组合类型。Partial 将所有属性变为可选，Required 将所有属性变为必需，Pick 从类型中选取部分属性，Omit 从类型中排除部分属性。此外还有 Record、Exclude、Extract、ReturnType 等常用工具类型。掌握这些工具类型可以大幅减少重复的类型定义。",
        codeExample: {
          code: `interface User {
  id: string;
  name: string;
...
// frozenUser.name = "新名字"; // 错误: 只读属性`,
      ...
      },
      {
        ...
        title: "自定义工具类型",
        content:
          "除了内置工具类型，你还可以根据项目需求自定义工具类型。通过组合条件类型、映射类型和模板字面量类型，可以实现强大的类型转换。常见的自定义工具类型包括 DeepPartial（深层可选）、DeepReadonly（深层只读）、ValueOf（获取值类型）等。理解这些类型的实现原理有助于你编写更灵活的类型定义。",
        codeExample: {
...
type CleanSearchParams = NullableToOptional<SearchParams>;`,
  ...
  },
  {
    ...
    title: "高级类型",
    description:
      "探索 TypeScript 的高级类型系统，包括联合类型、交叉类型、条件类型、映射类型和 infer 关键字。",
    order: 7,
    icon: "Sparkles",
    sections: [
      {
        ...
        title: "联合类型与交叉类型",
        content:
          "联合类型使用竖线连接多个类型，表示值可以是其中任意一种。交叉类型使用 & 连接多个类型，表示值同时满足所有类型。联合类型常用于处理多种可能的输入，交叉类型常用于组合多个类型的属性。类型收窄是联合类型的关键技巧，通过类型守卫将联合类型缩小到具体类型。",
        codeExample: {
  ...
  switch (network.state) {
    ...
    case "connected":
      ...
      },
      {
        ...
        title: "条件类型",
        content:
          "条件类型类似于三元表达式，根据类型条件选择不同的类型结果。语法为 T extends U ? X : Y。当 T 是 U 的子类型时结果为 X，否则为 Y。分布式条件类型在联合类型上会自动分发，即 A | B extends U ? X : Y 会被拆分为 (A extends U ? X : Y) | (B extends U ? X : Y)。条件类型在创建灵活的工具类型时非常强大。",
        codeExample: {
...
type ErrorResult = ApiResult<Error>;`,
      ...
      },
      {
        ...
        title: "映射类型与模板字面量类型",
        content:
          "映射类型基于旧类型创建新类型，通过遍历属性名集合来生成新的属性定义。语法为 { [P in K]: T }，其中 K 通常是 keyof 某个类型。可以通过 +、- 修饰符添加或移除 readonly 和可选标记。模板字面量类型使用反引号语法，可以在类型层面进行字符串拼接，结合映射类型可以创建非常灵活的类型定义。",
        codeExample: {
...
type UserValidation = ValidationRule<Person>;`,
      ...
      },
      {
        ...
        content:
          "infer 关键字用于在条件类型中声明一个待推断的类型变量。它只能出现在 extends 子句中。TypeScript 会根据实际类型自动推断 infer 声明的变量。常见的应用包括提取函数的返回值类型、参数类型、Promise 的值类型等。infer 在实现递归类型和复杂类型转换时是不可或缺的工具。",
        codeExample: {
...
type DbParams = ConstructorParams<typeof DatabaseConnection>;`,
  ...
  },
  {
    ...
    title: "模块与命名空间",
    description:
      "了解 TypeScript 的模块系统、命名空间和声明文件的使用方法。",
    order: 8,
    icon: "Package",
    sections: [
      {
        ...
        content:
          "TypeScript 使用 ES 模块系统，通过 export 导出和 import 导入来组织代码。可以导出变量、函数、类、接口和类型别名。默认导出使用 export default，每个模块只能有一个默认导出。命名导出可以有多个，使用花括号导入。TypeScript 还支持类型专用导入导出，使用 type 修饰符明确表示只导入类型信息。",
        codeExample: {
          code: `// logger.ts
...
// import type { LogEntry } from "./logger";`,
      ...
      },
      {
        ...
        title: "命名空间",
        content:
          "命名空间使用 namespace 关键字定义，用于组织代码和避免命名冲突。在 ES 模块出现之前，命名空间是 TypeScript 中主要的代码组织方式。现在推荐使用 ES 模块，但命名空间在迁移旧代码和定义全局类型时仍有用武之地。命名空间支持嵌套，可以使用三斜线指令引用其他文件中的命名空间。",
        codeExample: {
...
const ageErrors = Validation.validateAll(25, [
  new Validation.NumberValidator(0, 150, "年龄必须在0到150之间"),
      ...
      },
      {
        ...
        title: "声明文件",
        content:
          "声明文件以 .d.ts 为扩展名，用于描述 JavaScript 代码的类型信息，而不包含实际的实现代码。当你使用没有类型定义的第三方 JavaScript 库时，需要声明文件来获得类型提示。可以通过 DefinitelyTyped 仓库安装社区维护的声明文件（@types/包名），也可以自己编写声明文件。declare 关键字用于声明全局变量、函数、类和模块。",
        codeExample: {
...
// npm install -D @types/node`,
      ...
      },
      {
        ...
        title: "模块解析与配置",
        content:
          "TypeScript 支持多种模块解析策略，包括 Node.js 经典解析、Node 解析和 Bundler 解析。moduleResolution 配置决定了 TypeScript 如何查找模块。paths 配置可以设置路径别名，baseUrl 设置模块解析的基础路径。了解模块解析机制有助于解决导入路径问题和配置项目结构。",
        codeExample: {
          code: `{
  "compilerOptions": {
    "module": "ESNext",
